...
let lastTime;
let moveTimer;
const BASE_MOVE_INTERVAL = 100; // Base snake speed (lower = faster)
let currentMoveInterval = BASE_MOVE_INTERVAL; // Current speed setting
let speedMultiplier = 1.0; // Speed multiplier from slider
...
// Add speed slider event listener outside of initGame to ensure it's only bound once
speedSlider.addEventListener('input', function () {
    speedMultiplier = parseFloat(this.value);
    currentMoveInterval = BASE_MOVE_INTERVAL / speedMultiplier;
    updateSpeedDisplay();
});
...
function initGame() {
    ...
    // Initialize snakes, food, etc.
    snake = {
        segments: [
            { x: Math.floor(COLS / 3), y: Math.floor(ROWS / 2) }
        ],
        maxLength: 5,
        direction: { x: 1, y: 0 },
        nextDirection: { x: 1, y: 0 },
        color: SNAKE_COLORS.player
    };
    ...
    document.getElementById('aiStatus').textContent = isPlayerAI ? "AI MODE ON" : "AI MODE OFF";
    document.getElementById('aiStatus').style.opacity = isPlayerAI ? 1 : 0;
...
function changeDirection(event) {
    ...
    // Prevent AI from taking control if player makes a move
    if (isPlayerAI && (event.key === 'ArrowUp' || event.key === 'ArrowDown' ||
        event.key === 'ArrowLeft' || event.key === 'ArrowRight' ||
        event.key === 'w' || event.key === 'a' ||
        event.key === 's' || event.key === 'd')) {
        isPlayerAI = false;
        document.getElementById('aiStatus').textContent = "AI MODE OFF";
        document.getElementById('aiStatus').style.opacity = 0;
    }
    ...
    // Update snake's next direction
    if (snake && dx !== undefined && dy !== undefined) {
...
function startGame() {
    ...
    // Apply current speed value from slider
...
// Set up speed slider initial value and update display on page load
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Make sure the speedStatus element is visible
...
function updateSpeedDisplay() {
    ...
    speedStatus.textContent = speedMultiplier.toFixed(1) + 'X SPEED';
    speedStatus.style.opacity = 1;

    // Update speed status color based on speed
    if (speedMultiplier <= 1.5) {
        speedStatus.style.color = '#4CAF50'; // Green for normal speed
    } else if (speedMultiplier <= 2.5) {
        speedStatus.style.color = '#FFD700'; // Yellow for medium speed
    } else {
        speedStatus.style.color = '#FF6347'; // Red-orange for fast speed
    }
}
